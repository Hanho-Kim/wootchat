...
function init() {
  ...
  var apiPath = window.location.protocol + "//" + window.location.hostname + ":8000/api/v1/get/chatInfo";
...
function loadMessages() {
  ...
  var callback = function(snap) {
    ...
    if(String(data.time).slice(0,13) != String(lastChatDataTime).slice(0,13)){
      $(messageListElement).append($.parseHTML('<div class="chat-item chat-item-notification-time">' +
                                                  '<div class="chat-notification">' +
                                                    '<span>' + String(data.time).slice(0,13) + ' ' + data.timeWeekLabel + '</span>' +
                                                  '</div>' +
                                                '</div>'));
    }
...
function loadMessagesOnceInfiniteScroll( keyFrom, count ) {
  ...
  var callback = function(snap) {
    ...
    $.each(Object.keys(array),function(index,key){
      ...
      if(String(data.time).slice(0,13) != String(lastChatDataTimeInfiniteScroll).slice(0,13)){
...
function saveMessage(messageText) {
  ...
  return firebase.database().ref(chatConfig.firebase.instancePath).push({
    ...
    timeWeekLabel : getDateWeekLabel(time),
    notification  : false
...
function displayMessage(key, username, uid, text, avatarUrl, avatarColor, time, notification, imageUrl, appendBool) {
  ...
  div.querySelector('.time').textContent              = String(time).slice(14,-3);
